...
var primaryTags = ['background',
					'background-color',
					'-webkit-gradient',
					'moz-linear-gradient'];
					
/*
	Parse text, find similar, sort if necessary, then display. 
*/  
function getLines() {
	...
	for (var i = 0; i < lines.length; i++) {
		var line = getHex(lines[i]);
		if (line) {
			for (var c = 0; c < line.length; c++) { 
				var color = new Color(new RGBColor(line[c]));
	...
	colors = findSimilar(colors, !isNaN(parseInt($('#threshold').val(), 10)) ? parseInt($('#threshold').val(), 10) : 0 );
	
	//find most relevant colors, excluding similar colors
	calculatePrimary(colors);
	
	//sort if requested
	//if ($('#sort').is(':checked')) {  colors.sort(sortColorsHsl); }
	if ($('#sort').is(':checked')) {  colors = sortColorsRgb(colors); }
	
	var ellapsed = new Date().getTime() - start;
	
	//display primay colors
	var displayedCount = display(colors);
...
*/
function calculatePrimary(colors) {
...
*/
function findSimilar(colors, threshold) {
	var newColors = [];
	for (var c = 0; c < colors.length; c++) {
		if(colors[c]) {
					...
					if (comp !== c) { //dont compare itself
						if (color.toHex() !== compColor.toHex()) { //not the same color
							if (Math.abs(color.r - compColor.r) < threshold && Math.abs (color.g - compColor.g) < threshold && Math.abs(color.b - compColor.b) < threshold) { //check similarity
								if (!arrayContainsColor(colors[c].similar, colors[comp])) {	//check if similar color already added
									colors[c].similar.push(colors[comp]); //add to similar
								} 
								if (!arrayContainsColor(colors[comp].similar, colors[c])) {
			...
			colors[c].similar = sortColorsRgb(colors[c].similar);
...
*/
function arrayContainsColor(array, color) {
...
/*
	Sort colors by hue, saturation, then lightness
	@param a Color First color
	@param b Color Second color
*/
function sortColorsHsl(a, b) {
	if (a.color.l === b.color.l) {
		if (a.color.h === b.color.h) {
			return a.color.s - b.color.s;
		} else {
...
function display(colors) {
	...
	for (var c = 0; c < colors.length; c++) {
		if (colors[c].primary) {
			...
			out.className = "color";
			$(out).tipTip({content: getDisplayHTML(colors[c])});
			output.appendChild(out);
			var lineNumber = colors[c].line;
			out.onmouseover = getMouseHandler(lineNumber);
			out.onmouseout = getMouseHandler(lineNumber, true);
			displayedCount++;
		}
	}
...
function getMouseHandler(line, out) {
	return function() { 
		lines[line-1].style.backgroundColor = out ? "" : "#f0ff00 !important";
		lines[line-1].scrollIntoView(true);
		if(lines[line-8]) {
			lines[line-8].scrollIntoView(true);
		}
	}
}
...
/*
	...
	@return match Array of matching strings
*/
